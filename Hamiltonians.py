...
def H0_full(e_s,e_l,e_c,d_l):
    ...
    H_s= tensor(identity(2),identity(2),identity(d_l),e_s*matrix_element(1,1,2)) # system
...
def H0_k(e_s,e_l,e_c,d_l,k):
    if k>d_l-2 or k<0:
        print("error: k out of bounds, 0<= k<d_l -1")
        return()
    H_s= tensor(identity(2),identity(2),identity(d_l),e_s*matrix_element(1,1,2)) # system
    H_l= tensor(identity(2),identity(2),Qobj(np.sum([n*e_l*matrix_element(n,n,d_l).full() for n in range(0,k+1)],axis=0)),identity(2)) #levels until k
    H_l+= tensor(identity(2),identity(2),Qobj(np.sum([(e_l*(n-1)+e_s)*matrix_element(n,n,d_l).full() for n in range(k+1,d_l)],axis=0)),identity(2)) #levels until d_l-1
    H_h= tensor(identity(2),(e_c+e_l)*matrix_element(1,1,2),identity(d_l),identity(2)) # hot bit
    H_c= tensor(e_c*matrix_element(1,1,2),identity(2),identity(d_l),identity(2)) # cold bit
    H0=H_l+H_h+H_c+H_s
    return(H0.to(core.data.CSR))
...
def HI_k(g_sl,g_ml,d_l,k):
    if k>d_l-2 or k<0:
        print("error: k out of bounds, 0<= k<d_l -1")
        return()
    H_al= g_sl*tensor(identity(2),identity(2),matrix_element(k,k+1,d_l),matrix_element(1,0,2)) # system-ladder interaction
    H_ml= g_ml*tensor(matrix_element(0,1,2),matrix_element(1,0,2),
                      Qobj(np.sum([qzero(d_l) if n == k else Qobj(matrix_element(n,n+1,d_l).full()) for n in range(0,d_l-1)] ,axis=0))
                      ,identity(2)) # ladder-bath interaction below k level
    # H_ml+= g_ml*tensor(matrix_element(0,1,2),matrix_element(1,0,2),
    #                    Qobj(np.sum([matrix_element(n,n+1,d_l).full() for n in range(k+1,d_l-1)] ,axis=0))
    #                   ,identity(2)) # ladder-bath interaction above k level
    HI_0=H_al+H_ml
    HI_0=HI_0+HI_0.dag()
    return(HI_0.to(core.data.CSR))
